...
public class Cryptographer {
    ...
    public static String encrypt(String stringForEncrypt, int cryptPower) {
        ...
        for (int i = 0; i < stringForEncrypt.length(); i++) {
            ...
            if ((charASCIIForEncrypt >= ASCII_CODE_a) & (charASCIIForEncrypt <= ASCII_CODE_a + ALPHABET_COUNT)) {
                resultString += (char) (ASCII_CODE_a + charNumberInAlphabet);
            } else if ((charASCIIForEncrypt >= ASCII_CODE_A) & (charASCIIForEncrypt <= ASCII_CODE_A + ALPHABET_COUNT)){
                resultString += (char) (ASCII_CODE_A + charNumberInAlphabet);
            } else {
